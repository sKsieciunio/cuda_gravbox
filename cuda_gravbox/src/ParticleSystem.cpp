...
{
    if (m_cuda_res_pos_x)
        CUDA_CHECK(cudaGraphicsUnregisterResource(m_cuda_res_pos_x));
    if (m_cuda_res_pos_y)
        CUDA_CHECK(cudaGraphicsUnregisterResource(m_cuda_res_pos_y));
    if (m_cuda_res_vel_x)
        CUDA_CHECK(cudaGraphicsUnregisterResource(m_cuda_res_vel_x));
    if (m_cuda_res_vel_y)
    ...
    if (m_useCUDA)
    {
        if (m_particles.prev_position_x)
            CUDA_CHECK(cudaFree(m_particles.prev_position_x));
        if (m_particles.prev_position_y)
            CUDA_CHECK(cudaFree(m_particles.prev_position_y));
    }
    else
    {
        if (h_prev_position_x)
            free(h_prev_position_x);
        if (h_prev_position_y)
...
void ParticleSystem::initializeParticleData(int windowWidth, int windowHeight, float particleRadius, SpawnMode mode)
{
    ...
    else if (mode == SpawnMode::DISK_CORNER || mode == SpawnMode::DISK_CENTER_EXPLOSION)
    {
        ...
        float spacing = particleRadius * 2.0f + 0.1f;
    ...
    else if (mode == SpawnMode::DISK_VS_WALL)
    {
        ...
        for (int i = 0; i < m_particleCount; i++)
        {
            ...
            else
            {
                ...
                py = particleRadius + row * wallSpacing;
